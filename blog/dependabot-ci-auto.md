# CI を通した安全な Dependabot の自動マージと自動クローズの構築方法
...
このドキュメントは以下のような人を対象としています

- Dependabot の PR 運用を改善したい
- branch protection を維持したまま自動化したい
- GitHub Actions + GitHub App の構成を実務で使いたい

また、この構成では以下はあえて行っていません

- required checks の bypass
- CI の結果を無視したマージ
- すべての Dependabot PR の自動処理
...
- PR作成 → 種別判定 → majorは閉じる / minor・patchは待機 → CI成功後にマージ
