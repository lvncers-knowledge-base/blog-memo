...
実は今も共存しているんですが、将来的には Github へ完全移行したいと思っています。
Projects などの便利な機能もあって、マイルストーンとの紐付けやチケット発行など、プロジェクトマネジメントがしやすいからです。
...
これから教訓として、AIの実行には許可を求めない `Auto-Run Mode` で自走させていましたが、それを外した上で、Cursor Hooks によってコマンド実行を強制的に弾いていました。
